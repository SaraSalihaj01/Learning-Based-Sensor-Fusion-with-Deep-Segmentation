...
class RGBMaskDataset(Dataset):
    ...
    def __init__(self, images_dir, masks_dir, img_size=256, limit=None):
        ...
        # Search for all .jpg images recursively in the nyu2_train folder
        img_paths = sorted(glob.glob(os.path.join(images_dir, "**", "*.jpg"), recursive=True))
        if limit:
            img_paths = img_paths[:limit]
        
        # Pair each .jpg with its corresponding _mask.png
        ...
        if not self.items:
            raise RuntimeError(
                ...
                "Check if masks in output folder have the suffix '_mask.png'."
            )
